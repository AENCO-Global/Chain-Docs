...
Interaction with API WebSockets in :doc:`AEN-SDK <../sdk/overview>` is done with **Listeners**.
...
The status channel notifies when a transaction related to an address rises an :doc:`errors <status-errors>`. The message contains the error message and the transaction hash.


Continue: :doc:`Status errors <status-errors>`.
