...
This guide will walk you through a step-by-step installation of the required tools to start developing on AEN Catapult.
...
An account is a key pair (private and public key) associated to a mutable state stored in the AEN blockchain. In other words, you have a deposit box on the blockchain, which only you can modify with your key pair. As the name suggests, the private key has to be kept secret at all times. Anyone with access to the private key, ultimately has control over the account.
...
Finally, the account address is generated with the public key, following the AEN blockchain protocol. Share this address instead of the public key, as it contains more information, such as a validity check or which network it uses (public, testnet or private).

:doc:`AEN-CLI <../cli/overview>` conveniently allows you to perform the most commonly used commands from your terminal i.e. using it to interact with the blockchain, setting up an account, sending funds, etc.

1. Install AEN-CLI using ``npm``.
...
.. code-block:: bash
    ...
    $> AEN-cli account generate --network MIJIN_TEST --save --url http://localhost:3000

The ``network flag`` is set to MIJIN_TEST. Test network is an alternative AEN blockchain used for development and testing purposes.

Use ``save flag`` to store the account on your computer. AEN-CLI uses stored account to sign the transactions that you start.
...
*******************************
What is AENC and how to get it?
-------------------------------

The underlying cryptocurrency of the AEN network is called **AENC**. Every action on the AEN blockchain costs AENC, in order to provide an incentive for those who validate and secure the network.

Let’s use an account which already has AENC. We will need it to register the namespace and mosaic.
...
2. Under the section ``AENesis_addresses``, you will find the key pairs which contain AENC.

3. Load the first account as a profile in AEN-CLI. This account identifies the company.

.. code-block:: bash

    $> AEN-cli profile create

    Introduce network type (MIJIN_TEST, MIJIN, MAIN_NET, TEST_NET): MIJIN_TEST
    Introduce your private key: 41************************************************************FF
    Introduce AEN 2 Node URL. (Example: http://localhost:3000): http://localhost:3000
...
2. Install AEN-sdk and rxjs library.

.. code-block:: bash

    $> npm install AEN-sdk rxjs

3. AEN-sdk is build with TypeScript language. It is recommended to use **TypeScript instead of JavaScript** when building applications for AEN blockchain.
...
3. Add AEN-sdk and reactive library as a dependency.

.. code-block:: java

    dependencies {
        compile "io.AEN:sdk:0.9.1"
...
1. Create a new project using a C# IDE. If it is Visual Studio, use the Package Manager Console to install the AEN-sdk.

2. Open the ``Tools > NuGet Package Manager > Package Manager Console`` menu command.

3. Enter AEN-sdk and reactive library packages names in the terminal.

.. code-block:: bash

    $> Install-Package AEN-sdk
